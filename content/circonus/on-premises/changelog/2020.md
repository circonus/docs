...
* (Stratcon) Fix potential crash that can occur when two or more nodes in a
  snowth cluster are down. Issue was caused by concurrent writes to a hash
  table.
* (Stratcon) Fix potential infinite recursion bug that could cause crashes if
  stratcond is unable to write to any node in the snowth cluster for an
  extended period of time.
* (Stratcon) Add the ability to write data out to jlog instead of to a swimlane
  cluster if the data is coming from a public broker. This will prevent data
  that belongs in a swimlane from causing public brokers to back up if a
  swimlane cluster is having issues.
...
* Fix: `/irondb_cluster` sql queries to remove join which was not returning
  clusters not associated with an account.
