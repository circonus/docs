...
* `BROKER_IP` : The IP address that Circonus should use to contact this broker.
  ...
  above.
* `BROKER_NAME` : A user-friendly alias for this broker that will be displayed in the Circonus UI.  Spaces are allowed but must be quoted,  e.g., `BROKER_NAME="Friendly Neighborhood Broker"`.  The default broker name is the local hostname.  If a conflicting broker name is supplied, explicitly or implicitly, then a random non-conflicting broker name will be generated and used.
